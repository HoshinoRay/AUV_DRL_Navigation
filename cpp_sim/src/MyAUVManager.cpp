...
: SimulationManager(stepsPerSecond, sf::SolverType::SOLVER_SI, sf::CollisionFilteringType::COLLISION_EXCLUSIVE)
{
    ...
    dataCollector->init("../logs", "GeneralMission_");
...
void MyAUVManager::BuildScenario()
{
    ...
    CreateLook("propeller",  sf::Color::RGB(1.f, 1.f, 1.f), 0.3f, 0.f, 0.f,
               sf::GetDataPath() + "propeller_tex.png");
    ...
    // Thruster placement geometry (metres)
    double inX = 0.33;   // inner ring fore-aft offset
    double inY = 0.137;  // inner ring lateral offset
    double outX = 0.45;
    double outY = 0.60;
    double zLevel     = -0.0;
    ...
    phy.buoyancy = false;
    sf::Box*       hullB    = new sf::Box("RootDummy", phy, sf::Vector3(0.001, 0.001, 0.001), sf::I4(), "Dummy", "black");
    ...
    for (int i = 0; i < 8; i++) {
        vertDucts.push_back(new sf::Polyhedron(
            "DuctVert" + std::to_string(i), phy,
            sf::GetDataPath() + "duct_hydro.obj", sf::Scalar(1), sf::I4(), "Dummy", "black"));
    }
    ...
    std::shared_ptr<sf::Polyhedron> propeller = std::make_shared<sf::Polyhedron>(
        "Propeller", phy, sf::GetDataPath() + "propeller.obj", sf::Scalar(1), sf::I4(), "Dummy", "propeller");
    for (size_t i = 0; i < thrusterNames.size(); ++i) {
        auto rotorDynamics = std::make_shared<sf::MechanicalPI>(1.0, 10.0, 5.0, 5.0);
        auto thrustModel   = std::make_shared<sf::FDThrust>(0.20, 0.48, 0.48, 0.05, true, getOcean()->getLiquid().density);
...
void MyAUVManager::SimulationStepCompleted(sf::Scalar timeStep)
{
    ...
    if (robot) {
        for (const std::string& name : this->thrusterNames) {
            sf::Thruster* th = dynamic_cast<sf::Thruster*>(robot->getActuator(name));
            state.motors.push_back(th ? th->getSetpoint() : 0.0);
        }
    }
